...
# PWA 아이콘 PNG 재생성 (web/public/icon-source.svg 수정 시)
cd web && node scripts/gen-icons.mjs   # sharp 사용. 산출물(pwa-*.png) 커밋함
...
- 매니페스트·SW 설정은 `web/vite.config.ts`. 아이콘은 `web/public/pwa-{192,512}.png`+`pwa-maskable-512.png`(원본 `icon-source.svg`).
